...
class MapContainer extends React.Component{
    ...
    onInfoWindowOpen(props, e) {
        const button = (<button onClick={e => { this.props.addFavoritEvent(e, this.state.selectedPlace.eventObj) }}>"Add!❤️"</button>);
    ...
    displayMarkers = () => {
        ...
        return this.props.events.map((oneEvent, index ) => {
            ...
            return <Marker key={index} id={oneEvent.id} eventObj={oneEvent} position={{
                ...
                url={oneEvent.url}
                icon={
                    this.markerIconHendler(oneEvent.category_id)
                }
            />
               })
    }
